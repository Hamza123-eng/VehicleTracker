...
bool MPU6050::begin(mpu6050_dps_t scale, mpu6050_range_t range, int mpua)
{
   ...
   //
    ...
    i2c_driver_install(0, conf.mode, 512, 512, 0);
//
...
Vector MPU6050::readRawAccel(void)
{
  ...
  i2c_master_write_read_device(0, mpuAddress, &reg, 1,value, 6, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
...
{
   uint8_t value;
    i2c_master_write_read_device(0, mpuAddress, &reg, 1, &value, 1, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
  return value;
}
...
void MPU6050::writeRegister8(uint8_t reg, uint8_t value)
{
    ...
    i2c_master_write_to_device(0, mpuAddress, write_buf, sizeof(write_buf), I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
...
{
    int16_t value;
     i2c_master_write_read_device(0, mpuAddress, &reg, 1, (uint8_t *)&value, 1, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
    return value;
}

void MPU6050::writeRegister16(uint8_t reg, int16_t value)
{
     uint8_t write_buf[3] = {reg, (uint8_t)(value >> 8),(uint8_t)value};
      i2c_master_write_to_device(0, mpuAddress, write_buf, sizeof(write_buf), I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
}
